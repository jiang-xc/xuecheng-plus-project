...
public class MediaFileServiceImpl implements MediaFileService {
    ...
    public RestResponse<Boolean> checkfile(String fileMd5) {
        ...
        if (mediaFiles != null) {
            ...
            try {
                ...
                log.error("去minio中查询文件出错，异常为：{}", e.getMessage());
            }
        }
    ...
    public RestResponse<Boolean> checkchunk(String fileMd5, int chunkIndex) {
        ...
        try {
            ...
            log.info("去minio中查询不到文件：{}", e.getMessage());
        }
        //分块未存在
        return RestResponse.success(false);
    }
    ...
    public RestResponse<Boolean> uploadChunk(String fileMd5, int chunk, String localChunkFilePath) {
        ...
        if (!b) {
            log.error("上传分块文件出错。。。分块文件路径（本地）：{}", chunkFilePath);
    ...
    public RestResponse<Boolean> mergechunks(Long companyId, String fileMd5, int chunkTotal, UploadFileParamsDto uploadFileParamsDto) {
        ...
        //文件入库
        currentProxy.addMediaFilesToDb(companyId, fileMd5, uploadFileParamsDto, bucket_videos, mergeFilePath);
        //=====清除分块文件=====
